...
等效手動啟動時，需先確保 `redis://127.0.0.1:6379` 可用，並用 `EMORAPY_COMMIT_SHA=$(git rev-parse HEAD)` 啟動 backend；`CJ_COMMIT_SHA` 保留為 legacy fallback，避免本機 `/version.commitSha` 回 `unknown`。固定端口為 backend `3001`、主站 `5173`、Admin `5175`、Redis `6379`。
