...
| provider layer | SafeArea、Paper、QueryClient provider、auth/session bootstrap | M0 provider、auth/session cache preload 與 root shell 已具備；credential / session identity transition 會先旋轉 identity epoch、停用新私密讀取、cancel 及 remove 舊 user-scoped query，避免 logout、A→B login 或 late response 重用前一身份 cache；physical device gate 待補 |
| storage adapter | token / session / device metadata SecureStore adapter；current keys 為 `emorapy.auth.token`、`emorapy.session.id`、`emorapy.device.meta`、`emorapy.navigation.pendingLandingHref`、`emorapy.locale`，舊 `cj.*` 只作 lazy migration source | runtime adapter、auth/session bootstrap 與 key migration 已具備；physical device SecureStore evidence 待補 |
...
| Chat | 聊天室、邀請、先聊再判、private/shared lane、exact Analysis consent、未開始處理前的本人 approval 撤回、purpose-scoped capsule authorization 撤回及 chat handoff；只有 roleA 顯示證據選擇與建立 Analysis request，證據預設為零並逐則選擇共同訊息／已批准摘要，roleB 只查看 exact scope、批准或撤回本人批准；同一 request 的 approve / decline / revoke / submit / judgment handoff 共用單一 action lock；backend safety notice 依 request locale 寫入 `message.content`，App 只顯示該訊息 | chat / message / stream / judgment handoff | M3 |
...
| --- | --- | --- | --- |
| Storage | SecureStore adapter、auth/session bootstrap、Emorapy current key / legacy `cj.*` lazy migration 與 unit gate 已建立 | 封裝 Expo SecureStore，支援 token / session restore / clear / device metadata；current key 使用 `emorapy.*`，legacy `cj.*` 只作讀取遷移與清理；使用者 query 由單一 identity-scope helper 管理，credential clear 失敗時 fail closed，不重新啟用私密 query | 不得在 screen 中直接讀寫 SecureStore，不得長期保存 case / psych 正文；不得把 legacy `cj.*` key 當 current App storage contract；不得使用未綁定 identity epoch 的 user/private query key |
