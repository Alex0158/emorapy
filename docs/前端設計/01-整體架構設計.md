...
**框架與語言**：
- **React 19.x**：組件化開發，生態豐富
- **TypeScript 5.x**：類型安全，提升代碼質量
- **Vite 5.x**：快速構建，優秀的開發體驗

**UI組件庫**：
- **Ant Design 6.x**：企業級 UI 組件，開箱即用
- **自定義主題**：基於品牌色彩定制 Ant Design 主題

**狀態管理**：
- **Zustand 5.x**：輕量級狀態管理，簡單易用
- **TanStack React Query 5.x**：服務器狀態管理，緩存和同步

**路由管理**：
- **React Router 7.x**：聲明式路由，支持嵌套路由
...
│   │   │   ├── ChatBubble/           # v2.0: 訪談氣泡（iMessage 風格）
│   │   │   ├── FeedbackCard/         # v2.0: 洞察反饋卡片
│   │   │   ├── ConsentModal/         # v2.0: 知情同意彈窗
│   │   │   └── RichnessRing/         # v2.0: 豐富度環形視覺化
│   │   └── feedback/                # 反饋組件
│   │       ├── Toast/                # 提示
│   │       ├── Notification/         # 通知
│   │       └── Message/              # 消息
...
│   ├── store/                        # 狀態管理（Zustand）
│   │   ├── authStore.ts              # 認證狀態
│   │   ├── sessionStore.ts           # 快速體驗 Session
│   │   ├── caseStore.ts              # 案件狀態
│   │   ├── judgmentStore.ts          # 判決狀態
│   │   ├── reconciliationStore.ts    # 和好方案狀態
│   │   ├── executionStore.ts         # 執行追蹤狀態
│   │   ├── interviewStore.ts         # v2.0: 訪談狀態（SSE）
│   │   └── psychProfileStore.ts      # v2.0: 心理畫像狀態
│   │
│   ├── services/                     # API 服務
│   │   ├── api/                      # API 定義
│   │   │   ├── auth.ts               # 認證 API
│   │   │   ├── user.ts               # 用戶 API
│   │   │   ├── session.ts            # Session API（快速體驗）
│   │   │   ├── case.ts               # 案件 API
│   │   │   ├── judgment.ts           # 判決 API
│   │   │   ├── pairing.ts            # 配對 API
│   │   │   ├── reconciliation.ts     # 和好方案 API
│   │   │   ├── execution.ts          # 執行 API
│   │   │   ├── interview.ts          # v2.0: 訪談 API（含 SSE）
│   │   │   └── psychProfile.ts       # v2.0: 心理畫像 API
│   │   ├── request.ts                # 請求封裝（含攔截器）
│   │   └── sseRequest.ts             # v2.0: SSE 請求工具
│   │
│   ├── utils/                        # 工具函數
│   │   ├── format.ts                 # 格式化
│   │   ├── validate.ts               # 驗證
...
│   │
│   ├── router/                       # 路由配置
│   │   └── index.tsx                 # 路由定義（守衛見 ProtectedRoute/PublicRoute）
│   │
│   ├── App.tsx                       # 根組件
│   ├── main.tsx                      # 入口文件
...
│
├── /judgment               # 判決（需登錄）
│   └── /:id               # 判決詳情（含結果與接受/生成和好方案，由 Detail 頁統一承載）
│
...
│   └── /:planId/checkin   # 打卡
│
├── /interview              # v2.0: AI 訪談（需登錄 + 知情同意檢查）
│   ├── /:sessionId       # 訪談對話（SSE 即時串流）
│   └── /:sessionId/result # 訪談結果 / 反饋卡片
│
└── /profile                # 個人中心（需登錄）
    ├── /index             # 個人資料
    ├── /settings          # 設置
    ├── /pairing           # 配對管理
    └── /my-story          # v2.0: 我的故事（心理畫像總覽）
```
...
**知情同意路由**（v2.0）：`/interview/*` — 由訪談頁面 mount 時檢查 `psych_consent_given`，未同意則顯示 ConsentModal（無獨立 ConsentGuard 路由守衛組件，後端 `requireConsent` 中間件為最終防線）
**條件路由**：某些功能需要配對後才能使用
...
**文檔版本**：v3.0  
**創建日期**：2024年  
**最後更新**：2026-02-20（v3.0：新增 Interview/Chat、Interview/Result 頁面目錄、interviewStore/psychProfileStore、interview/psychProfile API 和 sseRequest 工具）
