...
| Release completion audit | 未完成 / 部分舊 identity evidence 已失效 | 帶本機 release env 執行 `release:completion:audit -- --json` 在 Emorapy identity 下 `failures=0`、`passed=11`、`blocked=12`；舊 iOS simulator、Android emulator/app/Maestro 與 native upload evidence 因 `com.cj.motherbearcourt` 被標記 stale ignored。`eas_project_id`、`expo_token`、`eas_android_build_artifact`、`sentry_ios_native_prebuild_configuration` 與 `telemetry_runtime_evidence` 已解除；剩餘需補 Apple / ASC 憑證、EAS iOS artifact、TestFlight、physical device、provider delivery 與 native crash runtime |
