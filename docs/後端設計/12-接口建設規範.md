...
**實現對齊**：與當前 `responseFormatter`、`errorHandler`、[03-API設計](./03-API設計.md) 一致；錯誤碼與 [03-API設計](./03-API設計.md) §錯誤碼 對照。
...
訪談系統使用 Server-Sent Events 進行流式通信，遵循以下規範：
...
X-Accel-Buffering: no  // Nginx 反向代理需要
```
...
|--------|-----------|------|
| `token` | `{ "text": "..." }` | 即時文字片段 |
| `metadata` | `{ "intent": "...", "target_domains": [...], ... }` | AI 元數據（不展示給用戶） |
| `safety_alert` | `{ "message": "...", "severity": "warning" }` | 安全警報（⚠️ 源碼實際 payload 為 `{ message, severity }`，無 `resources`；危機熱線內建於 SafetyAlert 組件，severity=critical 時顯示） |
| `error` | `{ "code": "...", "message": "..." }` | 錯誤 |
| `complete` | `{ "session_id": string, "status": string }` | 流結束 |

#### 雙通道格式（後端內部）【設計方案，尚未實現】

後端 AI 回應原始格式（設計方案）使用 `---METADATA---` 分隔符：

```
你提到了和母親的關係...（文字部分，即時流式展示）
---METADATA---
{"intent":"deepen_attachment","target_domains":["attachment"],"should_end":false,"safety_flag":false}
```

後端解析後轉換為上述 SSE 事件類型推送。

⚠️ **實現說明**：當前 AI 回應格式為單一 JSON（含 `text`/`intent`/`target_domains`/`should_end`/`safety_flag`/`safety_message`），由 `interview.service.ts` 解析後轉為 SSE 事件推送。`---METADATA---` 雙通道格式為設計方案，尚未實現。後端 respond 端點使用 **callback-based SSE**（`Promise<void>` + `onSSE` callback 參數），Controller 層將 callback 轉為 HTTP SSE 推送——**非 AsyncGenerator**。

#### 安全要求

1. **分隔符注入防護**：用戶輸入中的 `---METADATA---` 必須被 strip
2. **取最後一個分隔符**：防止 AI 被誘導生成分隔符
3. **JSON 解析失敗降級**：metadata 解析失敗時不影響文字展示

#### 斷線處理

- 後端偵測到 client 斷線後仍完成 AI call
- 將 AI 回應寫入 DB（`interview_turns`）
- 前端重連後通過 `GET /interview/:id` 載入歷史（回應含 turns）
...
| `NOT_FOUND` | 404 | 訪談 session 不存在**或**不屬於當前用戶。⚠️ 代碼統一使用 `NOT_FOUND`（設計中的 `SESSION_NOT_FOUND` / `SESSION_NOT_OWNED` 已合併） |
| `SESSION_COMPLETED` | 409 | session 已結束（含 `completed` 和 `abandoned`，統一使用此碼） |
| `MAX_TURNS_REACHED` | **422** | 已達最大 turn 數（硬限 25 輪，`INTERVIEW_MAX_TURNS`）。⚠️ 代碼返回 422（非 409） |
| `CONCURRENT_REQUEST` | 409 | session 正在處理中（mutex lock） |
| `TURN_TOO_FAST` | 429 | turn 間隔不足 3 秒 |
| `RATE_LIMIT_EXCEEDED` | 429 | 開始訪談頻率超限。⚠️ 代碼統一使用 `RATE_LIMIT_EXCEEDED`（設計中的 `START_RATE_LIMIT` 未被使用）。雙層限流：①中間件每用戶每小時 3 次（`INTERVIEW_START_RATE_LIMIT`，防濫用）；②業務邏輯每用戶每天 5 個 substantive session（`INTERVIEW_DAILY_SESSION_LIMIT`，僅計 ≥ 3 輪） |
| `AI_CALL_FAILED` | SSE error 事件 | AI 調用失敗（重試 3 次仍失敗後通過 SSE `error` 事件推送；interview 排除 429/401） |
| _(無專用碼)_ | 200 | polling `GET /:id` **始終返回 200**。⚠️ 設計曾規劃 `PROCESSING_NOT_DONE` (202) / `PROCESSING_FAILED` (500)，但代碼統一返回 200 + session 對象，前端從 `status` 判斷 |
| `VALIDATION_ERROR` | 422 | `end` 對非 in_progress / `retry` 對非 processing_failed 的 session |

---

**文檔版本**：v2.0  
**創建日期**：2024年  
**最後更新**：2026-02-21（v2.0：SSE 接口規範（callback-based 實現，非 AsyncGenerator）、雙通道格式設計說明、安全要求、新錯誤碼含 RATE_LIMIT_EXCEEDED 雙層限流）  
**維護者**：後端開發團隊
