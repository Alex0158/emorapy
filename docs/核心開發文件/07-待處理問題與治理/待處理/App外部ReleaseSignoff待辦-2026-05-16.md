...
| EAS project id | 已完成 | `mobile/app.json` 已連結 Expo project `@alexdev518/cj-mobile`，`expo.extra.eas.projectId=1a4308a8-70ff-4bbc-a5e0-3f2a060888bc`，`release:completion:audit` 顯示 `eas_project_id` passed |
...
| EAS Android production artifact | 已完成 | `eas build --platform android --profile production` 已產生 Android store production build，pass evidence 為 [App-EAS-Android-Release-2026-06-13T10-27-04-956Z.json](../../90-證據與盤點/環境與發版驗證/App-EAS-Android-Release-2026-06-13T10-27-04-956Z.json)；因 `eas.json` 使用 remote app version source，Android runner 在 run mode 以 EAS remote `versionCode=5` 查詢 finished build，本地 `android.versionCode` 不作 EAS artifact identity source |
...
| Release completion audit | 未完成 | `release:completion:audit` 明確列出 7 個 release blockers；`eas_project_id`、`expo_token`、`eas_android_build_artifact`、`telemetry_runtime_evidence` 與 `release_production_db_parity` 已通過，剩餘為 Apple submission credentials、App Store Connect API credentials、EAS iOS build artifact、TestFlight evidence、physical device evidence、push provider delivery 與 production environment native crash runtime |
