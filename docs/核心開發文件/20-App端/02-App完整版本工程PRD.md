...
**文檔類型**：App端規格
**覆蓋範圍**：完整 App 版產品目標、技術路線、需求 ID、平台差異、成功指標與開發前工程裁決
...
本文是 CJ 完整 App 版的工程級 PRD。它在 `00-跨端產品核心/01-產品PRD總章.md` 的通用級 PRD 之下，裁決 App 作為普通用戶產品端如何承接跨端核心能力。

本文完成完整 App 版工程 PRD，並作為後續 M0-M6 實作裁決來源。當前 `mobile/` 已有 M0 route shell 與 platform adapter skeleton，但不宣稱 Quick / Auth / Chat / Case 等 CJ App runtime 主流程已完成；完整 App 版落地路線見 [03-App完整版本開發Roadmap.md](./03-App完整版本開發Roadmap.md)。
...
| Release | EAS Build / EAS Submit / EAS Update；iOS TestFlight 先行，Android readiness 後置 |
...
| Capacitor | 上線快，但 CJ 需要 native storage、push、deep link、upload、stream recovery 與 touch-first interaction，不適合作為長期 App 主線 |

## 3. 產品目標

完整 App 版的目標是讓普通用戶在手機上完成 CJ 核心關係梳理流程，而不是把 Web 頁面包進原生殼。
...
| CJ-PRD-APP-006 | App SHOULD 承接 Profile / Interview / My Story，並支援 SSE foreground streaming 與中斷後恢復 | P1 | Profile + Interview milestone smoke |
| CJ-PRD-APP-007 | App SHOULD 承接 Chat room、invite、message stream、request judgment 與 judgment handoff | P1 | Chat milestone e2e |
| CJ-PRD-APP-008 | App SHOULD 分階段承接 Formal Case、Judgment、Repair Journey、Execution checkin / replan | P1 | Case + Repair milestone regression |
| CJ-PRD-APP-009 | App MUST 以 Push + Deep Link 作再進場入口時保留 backend notification 狀態與授權 gate | P0 | Push + Deep Link milestone smoke |
| CJ-PRD-APP-010 | App MUST 以 platform upload adapter 管理 ImagePicker / metadata normalize / upload handoff，不在 App 本地裁決 media authorization | P0 | Upload adapter tests |
| CJ-PRD-APP-011 | App MUST 建立 crash / error / version / request context 的最小 telemetry 規格，但不把 telemetry 視為 Admin UI | P1 | Release hardening evidence |
...
| 分組 | 用途 | 首輪 screen |
| --- | --- | --- |
| `(public)` | 未登入或匿名可進入的 shell | landing、quick entry、login、register、forgot password |
| `(app)` | 登入後主 shell | case home、chat home、profile、notifications |
| `quick` | 匿名快速判斷與結果回訪 | create、collaborative、result |
| `auth` | 登入、註冊、claim 回跳 | login、register、claim result |
...
| Metric ID | 指標 | 狀態 |
| --- | --- | --- |
| CJ-MET-APP-001 | App quick start -> result completion rate | 待建立基線 |
| CJ-MET-APP-002 | App result -> auth claim success rate | 待建立基線 |
| CJ-MET-APP-003 | App stream reconnect / replay success rate | 待建立基線 |
| CJ-MET-APP-004 | Push / notification landing -> target action completion rate | 待建立基線 |
| CJ-MET-APP-005 | Crash-free App sessions | 待建立基線 |
| CJ-MET-APP-006 | Native upload success / fallback rate | 待建立基線 |
...
1. `mobile/app` 模板入口被 CJ navigation skeleton 替換。
...
5. iOS TestFlight build 有可追溯證據；Android readiness 不阻斷 iOS 首發，但不得破壞 cross-platform codebase。
