...
| Release completion audit | 未完成 / 舊 identity evidence 已失效 | `release:completion:audit -- --json` 在 Emorapy identity 下 `failures=0`，但仍 blocked；舊 iOS simulator、Android emulator/app/Maestro、EAS Android 與 native upload evidence 因 `com.cj.motherbearcourt` 被標記 stale ignored。2026-06-20 起 `eas_project_id` 也要求 EAS full name binding 對齊，不再只看 UUID。`sentry_ios_native_prebuild_configuration` 已由 Emorapy iOS prebuild refresh 解除；`telemetry_runtime_evidence` 已由 `App-Telemetry-Runtime-2026-06-20T08-52-45-182Z.json` 解除；剩餘需補 EAS project full name 對齊、外部憑證、EAS iOS / Android artifact、TestFlight、physical device、provider delivery 與 native crash runtime |
