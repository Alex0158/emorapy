...
- `respond/skip` 現在只負責提交動作並返回 `202 accepted`；後端會把 `stream.started/delta/completed/persisted/cancelled` 發到 `interview_session` scope，前端草稿回覆與最終交接都只跟隨 `AI Stream`。
...
- `stream.cancelled` 的 snapshot / replay 也會在 `Interview` 頁面保留為可見的 cancelled draft，確保中止生成後重新掛載頁面不會把最後狀態吃掉。
...
4. `cancel` 後前端仍可看到最後一個 cancelled draft。  
5. `PROCESSING_FAILED` 可經 `retry` 回到處理中。  
