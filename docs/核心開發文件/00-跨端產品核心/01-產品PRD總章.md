...
| 需要長期個性化的用戶 | 想讓系統理解自身、關係背景與互動模式 | 目標能力是可查看、修正、撤回的 PersonalMemory，以及須雙方批准 exact version 的 JointMemory；目前尚未實作 cross-case memory、不得自動重用 raw transcript |
...
| EMO-PRD-COM-003 | 先聊再判 | 用戶可先在明確區分的私人與共同空間建立語境，只有在資料擁有人批准 audience / purpose 後，相關材料才可進入共同調解或梳理結果 | F07 / F04、P04 |
| EMO-PRD-COM-004 | 讓系統更懂你 | 用戶可選擇提供訪談、個人與關係背景；私人背景可改善本人支持與共同流程方式，但不得未經批准成為共同事實、診斷、可信度、責任或正式結論的依據 | F06 / F08 / F09、P03 |
...
`EMO-PRD-COM-003` 與 `EMO-PRD-COM-004` 共用以下產品不變式：

1. AI 可以在隔離的 Private Analyst 層讀取本人已授權的私人歷史，並私下協助本人。
2. 私人內容只有在 owner 明示批准 `shared_mediation_adaptation`，且所有 active participants 接受不可歸因的動態流程調整模式後，才可轉成無原因、不可反推秘密的程序控制；任一方不接受時只使用 universal trauma-informed baseline。
3. Shared Mediator 不得讀私人原文或原因；需要共同討論的內容必須先形成由本人預覽、編輯及批准的 versioned Context Capsule。
4. 正式梳理的事實、可信度、責任、比例與結論只能由共同可見材料或批准用於該次梳理的 capsule 支持；在固定同一組 shared evidence 下，private context on/off 的 Decision Core及 Universal Shared Renderer 正式共同結果必須 byte-for-byte 相同。shared mediation只可由已授權的 bounded、不可歸因、無原因 controls調整節奏／問題數量；私人背景只可支援本人可見的「給我的提醒」，不得改共同 findings。
5. 自傷、暴力、脅迫控制等內容走獨立 safety workflow；可以中止普通共同流程，但不得以保護為名把秘密轉成對另一方的公開指控。
6. Solo Repair 可在本人授權下使用其私人背景並只向本人輸出；雙方可見的 joint Repair 只可使用 shared evidence、雙方批准的共同背景及當前偏好。單方可覆寫的 RelationshipProfile 不等於雙方批准記憶。
7. A、B 的 private histories 不得同時進入同一 model request；每位 owner 的短期 bounded controls由隔離 Strategy Compiler 產生，再由 backend deterministic、minimum-disclosure merge。
8. PersonalMemory只供本人私下支援；JointMemory必須雙方 exact-version approval。raw transcript、責任比例、單方指控、安全標籤及人格／創傷診斷不得自動跨案件記憶。

正式架構決策見 `EMO-ADR-010`；channel、capsule、exact approval、evidence-only Judgment / Repair containment及 release hardening 由 `a685db36` 引入並部署至 Web/Backend Production，CI run `29246076104` 與 Production run `29246324704` 均成功。目前 exact Production SHA 已前進至 docs-only successor `main@8e93680`，Production run `29249811661` 成功且公開 `/version.json` 對齊。此 evidence只證明 containment、migration/backfill與三端 release gate。per-owner compiler、purpose split、雙層 adaptation consent、Safety Router 與 Web/App trust/safety surface 已在 2026-07-13 local candidate 實作，但尚未合併或部署；live two-party、Safety recovery、App native、cross-case memory 與 legacy data lifecycle仍未完成。最新邊界由 [Chat 私密上下文待辦](../07-待處理問題與治理/待處理/Chat私密上下文與共同調解隔離重構待辦-2026-07-12.md) 追蹤。
...
| `EMO-PRD-COM-003` | MUST | P0 | Chat 是高頻入口；private/shared audience、explicit handoff 與 exact-purpose authorization 若錯誤會直接造成跨參與者資料洩漏或秘密證據 | `06-接口描述/07-chat.md`、`業務流程整合.md`、兩角色 projection / stream / malicious-echo / chat e2e |
| `EMO-PRD-COM-004` | MUST | P0 | 訪談與 profile 提供長期個性化上下文，但私人資料不得因全域 consent 被擴大到共同責任判斷；跨案件重用需要 purpose / audience / correction / revoke | `04-共用機制/03-AI風險與安全治理基線.md`、`04-資料治理與隱私風險基線.md`、profile / Judgment context-policy tests |
