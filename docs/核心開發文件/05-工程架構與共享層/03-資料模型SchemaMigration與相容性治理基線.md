...
| Migration history | `backend/prisma/migrations/*/migration.sql` 與 `migration_lock.toml` 已存在；本分支新增 expand-only `20260712210000_add_chat_context_domain_foundation`，建立 Chat channel/context/analysis models，但刻意不 backfill legacy messages | migration source 已存在並已納入 release-blocking catalog；目標 DB 尚未套用，仍需當次 parity evidence |
