...
**文檔版本**：v2.12
**最後更新**：2026-07-13
**代碼基準**：`backend/src/routes/judgment.routes.ts`、`backend/src/routes/ai-stream.routes.ts`、`backend/src/services/judgment.service.ts`、`backend/src/services/chat-formal-analysis-claim.service.ts`、`backend/src/services/judgment-normalization.service.ts`、`backend/src/services/chat.service.ts`、`packages/api-client/src/m4.ts`、`frontend/src/services/api/judgment.ts`、`frontend/src/services/aiStream.ts`、`mobile/src/features/m4/api.ts`
...
- `generate` 命中 `ChatToCaseLink` 時，不論從 Chat orchestrator 或 public judgment route 進入，都必須先通過共用 formal provider claim：以 `ReadCommitted` 取得 ordered active roleA/roleB participant locks，再重讀 Safety Router。`pause_shared/crisis_support`或讀取失敗時必須在 evidence claim、stream、case status 與 external AI provider 之前 fail closed。
...
- 判決生成仍必須透過 `backend/src/utils/case-classifier.ts` 判斷產品流，但 `ChatToCaseLink` 命中的 chat-to-case formal path 只能使用該次共同可見或 exact-approved 的 evidence bundle；不得因當事人已登入、有全域 consent 或屬 user-bound case，就注入 `profileContext`、其他 `caseContext`、PersonalMemory 或 cross-case history。未來 owner-only Personal Guidance 必須是與共同梳理結果分離、只給本人的獨立 surface。
...
11. public `generate` 對 Chat-linked case 遇到 formal Safety blocked 時，evidence/stream/case update 與 AI provider invocation 必須全部為零。
...
| `POST /api/v1/judgments/generate/:id` | `CASE_NOT_EDITABLE`   | 422  | 提示共同梳理目前因安全狀態暫停       | 保持個人安全支持，不自動重試 |
