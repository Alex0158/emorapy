...
export class ReconciliationService {
  ...
  private buildJourneyContext(plan: PlanRecordForHydration, userId: string) {
    ...
    const forceSoloRepair = safetyPolicy.forceSoloRepair || repairEligibility.forceSoloRepair;
    return buildRepairJourneyContext({
      ...
      canInvite: safetyPolicy.canInvitePartner
        && repairEligibility.canInvitePartner
        && !!this.getOtherParticipantId(plan.judgment.case, userId),
      hasPartner: !!commitment.partner,
      isDualCommitted: !forceSoloRepair && commitment.is_dual_committed,
      statusReason: plan.repair_track?.status_reason ?? null,
      recommendedMode: forceSoloRepair ? 'solo' : commitment.recommended_mode,
    });
  }

  private buildPlanCtaState(plan: PlanRecordForHydration, userId: string) {
    const journeyContext = this.buildJourneyContext(plan, userId);
  ...
  private async ensureRepairTrack(plan: PlanRecordForHydration) {
    if (plan.repair_track) {
      return plan.repair_track;
    }

    const participantIds = this.getParticipantIds(plan.judgment.case);
    return prisma.repairTrack.create({
      data: {
        plan_id: plan.id,
        intent: plan.intent,
  ...
  private buildJourneyEntry(
    ...
    return {
      ...
      has_superseded_versions: versionSummary?.has_superseded_versions ?? false,
      journey_context: this.buildJourneyContext(current, userId),
    };
  }
  ...
  private async syncTrackCommitment(
    ...
    return prisma.repairTrack.update({
      where: { id: track.id },
      data: {
  ...
  private async buildPlansPayload(
    ...
    return {
      ...
      journey_entry: this.buildJourneyEntry(plans, userId, versionSummary),
  ...
  async generatePlans(judgmentId: string, input?: GeneratePlansInput, userId?: string) {
    ...
    if (!repairEligibility.canGeneratePlans) {
    ...
    if ((safetyPolicy.forceSoloRepair || repairEligibility.forceSoloRepair) && preferences?.invite_partner) {
    ...
    if (repairEligibility.forceSoloRepair) {
      const eligibilityContext = repairEligibility.reasons.join('；');
  ...
  async getPlanById(planId: string, userId: string) {
    ...
    return {
      ...
      journey_context: this.buildJourneyContext(plan, userId),
      invite_context: {
        ...
        can_invite: getProductSafetyPolicyForJudgment(plan.judgment).canInvitePartner
          && getRepairEligibilityForCase(plan.judgment.case).canInvitePartner
          && !!this.getOtherParticipantId(plan.judgment.case, userId),
      },
      cta_state: this.buildPlanCtaState(plan, userId),
  ...
  async respondPlan(
    ...
    if (partnerId && action !== 'paused' && safetyPolicy.canNotifyPartner && repairEligibility.canNotifyPartner) {
      ...
      const journeyContext = this.buildJourneyContext(refreshedPlan, partnerId);
      await notificationService.createIfEnabled(partnerId, {
        ...
        dedup_key: `repair_journey_${action}_${track.id}_${partnerId}_${now.toISOString()}`,
        ...
        payload: {
          ...
          repair_track_id: track.id,
          journey_status: action === 'committed' ? 'co_active' : refreshedPlan.repair_track?.status ?? track.status,
  ...
  async invitePartner(planId: string, userId: string) {
    ...
    const safetyPolicy = getProductSafetyPolicyForJudgment(plan.judgment);
    const repairEligibility = getRepairEligibilityForCase(plan.judgment.case);
    if (!safetyPolicy.canInvitePartner || !repairEligibility.canInvitePartner) {
    ...
    if (partnerId) {
      ...
      await notificationService.createIfEnabled(partnerId, {
        ...
        dedup_key: `repair_journey_partner_invited_${track.id}_${partnerId}_${invitedAt.toISOString()}`,
        priority: 'now',
        group_key: `repair_track_${track.id}`,
        payload: {
          plan_id: planId,
          ...
          journey_context: this.buildJourneyContext(partnerPlan, partnerId) as unknown as Prisma.InputJsonValue,
        },
      });
    }
    return {
  ...
  async startPlan(planId: string, userId: string) {
    ...
    let track = await this.ensureRepairTrack(plan);
    ...
    if (track.step_progresses.length === 0) {
      await prisma.repairStepProgress.createMany({
      ...
      await prisma.repairStepProgress.updateMany({
  ...
  async resumeTrack(trackId: string, userId: string) {
    ...
    await prisma.repairParticipantState.upsert({
      ...
      create: {
        ...
        commitment_status: 'committed',
        viewed_at: new Date(),
        committed_at: existingState?.committed_at ?? new Date(),
      },
      update: {
        commitment_status: 'committed',
        paused_at: null,
    ...
    await this.createTrackEvent(track.id, 'track_resumed', userId, {
