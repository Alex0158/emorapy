...
| M6 release hardening | 部分落地；release DB parity 與 telemetry runtime 是 release audit 證據槽，EAS / TestFlight / physical device / provider delivery / native crash runtime 仍受 strict release blocker 約束 |
...
| 可用證據槽 | release / production DB parity、telemetry runtime、native readiness、upload、OTel 與 native crash SDK configuration 可作 release-hardening 證據槽；外部 sign-off 仍以 strict audit 接受的 evidence 為準 |
| 仍是 blocker | EAS project id、Expo token、Apple submission credentials、App Store Connect API credentials、EAS iOS build artifact、TestFlight evidence、physical device evidence、Android native toolchain evidence、EAS Android build artifact、push provider delivery evidence、production native crash runtime evidence |
