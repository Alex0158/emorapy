...
flowchart TD
    ...
    invite_pending --> invite_declined["invite_declined: B拒絕/過期"]
...
- `POST /api/v1/chat/rooms/:roomId/messages`
  - A/B 發訊息
- `POST /api/v1/chat/rooms/:roomId/ai-reply`
  - 觸發 AI 回覆（v1 可先同步，v1.1 改流式）
...
- Prisma 模型已新增：
  ...
  - `chat_to_case_links`
- 新增初版 migration 草稿：
  - `backend/prisma/migrations/20260225120000_add_chat_v1_models/migration.sql`
- 新增聊天室服務與路由：
...
- 轉判決規則精煉（情緒段/事實段分層、缺口標記更完整）
- 前端串接聊天室 SSE 與判決狀態頁
- E2E 與壓測（多房間 SSE、長對話截斷；`request-judgment` / `accept-invite` 並發基準腳本 + gate 已補，尚待真實多實例環境執行）
- migration 上線流程演練（預檢 + 修復方案輸出腳本已補，尚待在 staging 實資料執行與留檔）
- 路由層 integration test 擴展（邀請 accept/decline + judgment-status 成功路徑）
