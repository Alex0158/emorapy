...
describe('InterviewInput', () => {
  it('應渲染 textarea 和送出按鈕', () => {
    ...
    expect(screen.getByRole('button', { name: 'interview.send' })).toBeInTheDocument();
  });
  ...
  it('輸入文字後點擊送出應呼叫 onSend 並清空輸入', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'interview.send' }));
  ...
  it('isStreaming 且有 onStop 時應顯示停止按鈕', () => {
    ...
    expect(screen.getByRole('button', { name: 'interview.stop' })).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'interview.stop' }));
  ...
  it('有 onSkip 且非 streaming 時應顯示跳過按鈕', () => {
    ...
    expect(screen.getByRole('button', { name: 'interview.skip' })).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'interview.skip' }));
