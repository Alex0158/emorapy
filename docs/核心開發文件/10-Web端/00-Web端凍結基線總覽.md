...
2. Browser local/session storage adapter；current Web quick session key 為 `emorapy_session_id`，locale key 為 `emorapy_locale`。`cj_session_id` / `cj_locale` 與更早 `mbc_session_id` / `mbc_locale` 只作 lazy migration source，讀到舊值時寫入新 key 並清理舊 key，不得再作 current storage contract。
