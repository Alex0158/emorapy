...
| iOS Release simulator refresh | Runner 已建立 / 本機 iOS 26.5 simulator runtime blocked | 2026-06-20 已新增 `npm --prefix mobile run ios:release-simulator:smoke` structured runner，並將 Expo SDK 55 patch dependencies 對齊到 `expo install --check` 通過；`xcode-select -p` 仍指向 `/Library/Developer/CommandLineTools`，但以 `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer` 已可使用 Xcode 26.5 與 `simctl`。最新 blocked run 顯示可用 simulator runtime 仍是 iOS 26.4，`xcodebuild` 對 `iPhone 17` 回報 `iOS 26.5 is not installed`，因此尚不能產生 Emorapy identity pass evidence。需在 Xcode > Settings > Components 安裝 iOS 26.5 platform / simulator runtime，或提供與 Xcode 26.5 可用 runtime 匹配的 simulator destination 後重跑 |
...
| iOS Release simulator evidence | 在 Xcode > Settings > Components 安裝 iOS 26.5 platform / simulator runtime，或提供與 Xcode 26.5 可用 runtime 匹配的 simulator destination；目前完整 Xcode 可用，但 iOS 26.5 runtime 缺失 | 重跑 `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer npm --prefix mobile run ios:release-simulator:smoke` 產 Emorapy identity simulator pass JSON |
