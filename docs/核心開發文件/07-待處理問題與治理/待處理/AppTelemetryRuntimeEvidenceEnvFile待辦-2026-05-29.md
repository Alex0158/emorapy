...
**文檔類型**：問題治理
**覆蓋範圍**：App M6 telemetry runtime release evidence 的本機 / CI env-file 執行路徑、證據產生與 completion audit 收斂
**取證代碼入口**：`mobile/scripts/run-telemetry-runtime-smoke.mjs`、`mobile/release.env.example`、`mobile/scripts/check-release-completion-audit.mjs`
**最後核驗 Commit**：`8313e37`
**最後核驗日期**：`2026-05-29`
<!-- CORE_DOC_AUDIT_METADATA:END -->

**狀態**：工程修正已落地；等待後端部署後重跑 release runtime pass evidence
**Owner**：Mobile / Ops / QA
...
5. 若 release API 接受 `POST /telemetry/events` 與 `POST /telemetry/otlp/v1/traces`，產出 `App-Telemetry-Runtime-*.json` 後，`release:completion:audit` 應能把 `telemetry_runtime_evidence` 從 blocker 移除。
...
| Backend/API | 不新增 API；沿用 `POST /api/v1/telemetry/events` 與 `POST /api/v1/telemetry/otlp/v1/traces`；production CORS 只對這兩個 App native telemetry ingest 端點允許無 `Origin` 請求，非白名單瀏覽器 `Origin` 仍 403 |
...
3. telemetry runtime evidence 只證明 release API 可接受 App safe telemetry event 與 CJ OTLP JSON trace ingest；不證明 native crash、push delivery、真機 cold start 或 TestFlight readiness。
...
正式 release API 仍需在 backend CORS 修正部署後重跑 `telemetry:runtime:smoke -- --run --release-env-file=release.env.local`，產出 `blocked=false` 的 `App-Telemetry-Runtime-*.json` 後，才能解除 `telemetry_runtime_evidence` release blocker。
