...
8. Case access and product-flow classification must not be reimplemented ad hoc from `case.mode`. Backend code must use `backend/src/utils/case-classifier.ts` for session-vs-user access decisions, product-flow labels, user-bound case query scopes, and case/judgment/execution product-flow reporting filters; docs must record any new flow classification rule before frontend, notification, analytics, or API behavior is changed.
