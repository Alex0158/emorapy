...
本文把 CJ 的工程架構從「目錄怎樣分」提升為「架構決策為什麼成立、由誰影響、如何驗證、何時需要重審」。它對標 ISO/IEC/IEEE 42010:2022 的架構描述思想與 C4 model 的多視圖表達方式，但不宣稱 CJ 已形成完整 Architecture Description 或 C4 圖集。
...
| C4 model | 用 Context / Container / Component / Code 分層描述軟件架構，便於跨產品、工程、運維角色共同理解 | 用於建立 CJ 的文字版架構視圖層級；正式圖集待建立 |
...
CJ 後續描述架構時，至少按以下層級表達。沒有圖時可以先用文字矩陣，但不得只寫「見目錄」。

| 視圖 | 必須回答 | 當前 CJ 入口 |
| --- | --- | --- |
| System Context | CJ 與使用者、Admin、瀏覽器、App、AI provider、DB、Redis、部署平台、告警通道的關係 | `README.md`、`03-管理端與平台治理/`、`04-共用機制/` |
...
| CJ-ADR-007 | Accepted | 架構、共享層、API contract、DB schema、AI runtime、安全邊界或 App adapter 的變更必須觸發 ADR 或更新本文決策登記 | 本輪新增治理 gate；後續若 ADR 數量增長，應拆分為獨立 ADR 目錄 |
...
| Flutter | 跨平台成熟，但會放棄現有 `@cj/contracts`、`@cj/api-client`、Web domain helper 與 TypeScript 測試資產 | 不作完整 App 主線 |
| Capacitor | 可快速包 Web，但 CJ 需要 SecureStore、Push、Deep Link、native upload、stream recovery 與 touch-first App flow | 只可作極短期 demo，不作正式產品方向 |
