...
| 需要長期個性化的用戶 | 想讓系統理解自身、關係背景與互動模式 | 可建立可查看、修正、撤回的 PersonalMemory；雙方共同使用的 JointMemory 必須雙方批准 exact version，raw transcript 不自動跨案件重用 |
...
正式架構決策見 `EMO-ADR-010`；channel、capsule、exact approval、evidence-only Judgment / Repair containment及 release hardening 已由 exact `main@a685db36` 部署至 Web/Backend Production，CI run `29246076104` 與 Production run `29246324704` 均成功。此 evidence只證明 containment、migration/backfill與三端 release gate，不代表 per-owner compiler、purpose split、Safety Router、live two-party、App native、cross-case memory 或 legacy data lifecycle已完成；最新邊界由 [Chat 私密上下文待辦](../07-待處理問題與治理/待處理/Chat私密上下文與共同調解隔離重構待辦-2026-07-12.md) 追蹤。
