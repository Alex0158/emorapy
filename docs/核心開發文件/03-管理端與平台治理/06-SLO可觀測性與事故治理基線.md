...
| Metrics | Backend / Admin / Prometheus 為主 | App telemetry 尚未建立，不得用 Web metrics 代表 App 穩定性 |
...
| CJ-OPS-GAP-005 | App telemetry / App smoke evidence 尚未建立 | App 版無法繼承 Web 運維結論 | App 能力落地前回查 App 測試證據接入基線 |
| CJ-OPS-GAP-006 | Chat / AI stream 有 metrics，但沒有產品級 target | AI 體驗退化只能被動排查 | 暫以 metrics + regression + Admin report 管控 |
| CJ-OPS-GAP-007 | 無 OpenTelemetry trace / span / baggage，request id 尚未形成 distributed trace | 跨服務、AI provider、DB、Redis 的慢路徑根因難以定位 | 先以 request id + logs + metrics 管控；導入 OTel 前不得宣稱 tracing |
...
6. Baseline Pending：未建立 p95/p99、error budget、App telemetry、OpenTelemetry trace 或 privacy detector 前，不得宣稱完整 SLO、SLA、SOC 或 incident response maturity。
