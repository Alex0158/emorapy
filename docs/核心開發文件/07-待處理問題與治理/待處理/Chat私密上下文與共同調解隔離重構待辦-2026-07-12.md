...
**狀態**：仍在待處理；`codex/private-context-consent-boundary` release candidate 已完成 Wave 0–3、Wave 4 containment、release migration/backfill/audit tooling 與本地全量驗證，尚待合併及正式 Production workflow；cross-case memory 與 legacy data lifecycle 另保持活躍
...
以下是核對 `origin/main@95fa8a9` 得出的歷史缺口，用來說明本分支修正來源；它不是目前 working tree 的實作狀態，也不代表 Production 已隨分支改變：
...
### 2.1 Current branch implementation

目前 `codex/private-context-consent-boundary` release candidate 基於 `origin/main@95fa8a9`，已具備以下尚待正式發布的實作：
...
1. release candidate 尚未合併；必須先取得 exact main SHA 的 push CI success，不能以 feature branch SHA 或本地 commit 代替。
2. `20260712210000_add_chat_context_domain_foundation` 與 `20260713090000_add_context_authorization_active_unique` 已加入 `RELEASE_BLOCKING_MIGRATIONS`，且本地 fresh DB parity 已通過；正式發布仍要保存 Production runtime migration / parity evidence。
3. chat channel backfill 已在本地 fresh DB 完成 dry-run、apply 與 idempotency 驗證；Production 真實資料仍要保存 row counts、orphans、legacy summary quarantine 及重跑結果 artifact，任何 blocker 都要停止 Web promote。
...
7. Production workflow 已固定為 `expand migration -> Railway backend -> chat-context dry-run -> apply -> read-only privacy audit -> staged Web/Admin -> exact-SHA verify -> promote -> release gate`，但 release candidate 階段尚未執行；migration deploy log、backfill counts 與 legacy privacy audit artifact 都是 release gate，不可因 workflow 已存在就宣稱已發布。
...
本 release candidate 已 additive 新增 channel、capsule、authorization、analysis request/approval endpoints，並擴充 shared contracts、`@emorapy/api-client`、Web 與 App 消費面；legacy room message / `included_message_ids` 路徑仍保留相容窗口。本地 DB backfill 已通過，Production backfill artifact、舊 client 相容觀察與 deprecation 仍未完成。
...
- Production workflow 順序固定為 expand migration → Railway backend → chat-context dry-run/apply/audit → staged Web/Admin → exact-SHA verify → promote → release gate；release candidate 階段尚未在 Production 執行。
