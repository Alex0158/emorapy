...
export function reduceDraftWithEvent(
  ...
  if (
    event.eventType === 'stream.created'
    || event.eventType === 'stream.queued'
...
export function isTerminalAIStreamStatus(status?: AIStreamStatus): boolean {
  return status === 'persisted' || status === 'failed' || status === 'cancelled';
}

export function isTerminalAIStreamEvent(event: AIStreamEvent): boolean {
  return (
...
export function getLatestAIStreamSnapshot(snapshots?: AIStreamSnapshot[]): AIStreamSnapshot | null {
  if (!Array.isArray(snapshots) || snapshots.length === 0) return null;
  return [...snapshots].sort((a, b) => b.lastSeq - a.lastSeq)[0] ?? null;
}

export function getLatestActiveAIStreamSnapshot(snapshots?: AIStreamSnapshot[]): AIStreamSnapshot | null {
  if (!Array.isArray(snapshots) || snapshots.length === 0) return null;
  return [...snapshots]
    .sort((a, b) => b.lastSeq - a.lastSeq)
    .find((snapshot) => !isTerminalAIStreamStatus(snapshot.status)) ?? null;
}
