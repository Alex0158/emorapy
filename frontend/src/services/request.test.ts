...
vi.mock('axios', () => ({
  default: {
    create: vi.fn(() => {
      const instance = ((config: unknown) => mockRequest(config)) as unknown as {
...
import { cancelAllRequests, cancelRequest, requestWithRetryWrapper } from './request';
...
describe('request', () => {
  ...
  describe('requestWithRetryWrapper', () => {
    ...
    it('shouldRetry 應僅對網路錯誤與 5xx 返回 true', async () => {
      ...
      const options = (requestWithRetry as ReturnType<typeof vi.fn>).mock.calls[0][1];
  ...
  describe('interceptors', () => {
    it('request interceptor 的 error handler 應原樣拒絕', async () => {
      ...
      await expect(onRequestError(new Error('req-fail'))).rejects.toBeInstanceOf(Error);
    });
    ...
    it('response success=false 應轉為拒絕錯誤', async () => {
      ...
      ).rejects.toMatchObject({ code: 'E1', message: 'common.requestFail' });
    });
    ...
    it('error interceptor 遇到取消請求應返回 REQUEST_CANCELED', async () => {
      await expect(onError({ __cancel: true, name: 'CanceledError' })).rejects.toMatchObject({
        code: 'REQUEST_CANCELED',
        isCanceled: true,
      });
    });

    it('400 + INVALID_SESSION_ID 應 clear + refresh 並用本地化 fallback warning', async () => {
      await expect(
        onError({
          response: {
            status: 400,
            data: {
              error: { code: 'INVALID_SESSION_ID', message: 'Session 已過期' },
            },
        ...
        message: 'error.session.expiredHint',
      });
      expect(mockSessionStoreState.clearSession).toHaveBeenCalled();
      expect(mockSessionStoreState.refreshSession).toHaveBeenCalledWith(true, undefined);
      expect(mockToastWarning).toHaveBeenCalledWith('common.sessionExpiredRefreshed');
    });
    ...
    it('400 + INVALID_SESSION_ID 應把失敗請求的 X-Session-Id 傳給 refresh', async () => {
      ...
      expect(mockSessionStoreState.refreshSession).toHaveBeenCalledWith(true, 'guest_sid_old');
    });
    ...
    it('400 + INVALID_SESSION_ID 應支援小寫 x-session-id header 並傳給 refresh', async () => {
      ...
      expect(mockSessionStoreState.refreshSession).toHaveBeenCalledWith(true, 'guest_sid_lower');
    });
    ...
    it('400 + INVALID_SESSION_ID 當 refresh 拋錯時應走 catch 顯示 error', async () => {
      mockSessionStoreState.refreshSession.mockRejectedValueOnce(new Error('refresh failed'));
      await expect(
        onError({
          response: {
            status: 400,
      ...
      expect(mockToastError).toHaveBeenCalledWith('error.session.expiredHint');
    });
    ...
    it('400 一般錯誤帶後端 message 時不應外露 raw message', async () => {
      await expect(
        onError({
          response: {
            status: 400,
            data: { error: { code: 'VALIDATION_ERROR', message: '請求參數錯誤' } },
            config: { url: '/cases/x', headers: {}, params: {} },
          },
          config: {},
          message: 'bad',
        ...
        message: 'common.validationError',
      });
      expect(mockToastError).toHaveBeenCalledWith('common.validationError');
    });
    ...
    it('401 + SESSION_EXPIRED 當 refresh 拋錯時應走 catch 顯示 error', async () => {
      mockSessionStoreState.refreshSession.mockRejectedValueOnce(new Error('refresh explode'));
      await expect(
        onError({
          response: {
            status: 401,
    ...
    it('401 非 session 錯誤帶後端 message 時不應外露 raw message', async () => {
      localStorage.setItem('token', 'token-x');
      history.pushState({}, '', '/auth/login');
      await expect(
        onError({
          response: {
            status: 401,
            data: { error: { code: 'UNAUTHORIZED', message: '未授權' } },
            config: { url: '/auth/me', headers: {}, params: {} },
          },
          config: {},
          message: 'unauth',
    ...
    it('500 帶後端 message 時不應外露 raw message', async () => {
      await expect(
        onError({
          response: {
            status: 500,
    ...
    it('503 應提示 serviceUnavailable', async () => {
      ...
      expect(mockToastError).toHaveBeenCalledWith('common.serviceUnavailable');
    });
    ...
    it('網絡錯誤應返回 NETWORK_ERROR', async () => {
      await expect(onError({ request: {}, message: 'network down' })).rejects.toMatchObject({
        code: 'NETWORK_ERROR',
      });
    });
