 ...
 *       → shadcn Accordion + Dialog + Badge + Tailwind + sonner + Lucide
...
import { ArrowLeft, MessageCircle, Trash2, History, BookOpen, Loader2, AlertCircle } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 md:px-6" role="main">
        ...
        {/* Disclaimer */}
        <p className="mb-6 max-w-2xl rounded-lg border border-border bg-muted/20 p-3 text-sm text-muted-foreground">{t('psychProfile.disclaimer')}</p>
        ...
        <div className="mb-8 rounded-xl border border-border bg-card p-6">
              ...
              {latestNarratives.map((narrative) => (
                <AccordionItem key={narrative.domain} value={narrative.domain} className="rounded-lg border border-border px-4">
                  <AccordionTrigger className="py-3">
                    <span className="text-sm font-medium text-foreground">{getDomainLabel(narrative.domain as PsychDomain)}</span>
                  </AccordionTrigger>
                        ...
                        <div className="space-y-2">
                          {insightsByDomain[narrative.domain].map((insight) => (
                            <div key={insight.id} className="rounded-lg border border-border bg-card p-3 space-y-2">
                              <p className="text-sm font-medium text-foreground">{insight.key}</p>
                              <p className="text-sm text-muted-foreground">{insight.value}</p>
                              {insight.evidence && <p className="rounded border-l-2 border-primary/25 py-1 pl-3 text-xs text-muted-foreground">「{insight.evidence}」</p>}
                            </div>
                          ))}
                        </div>
                      </div>
          ...
          <div className="mt-5 border-t border-border pt-4">
            <Button variant="outline" size="sm" onClick={handleStartChat}>
              <MessageCircle className="size-4" />{t('psychProfile.continueChat')}
            </Button>
          </div>
        </div>
