...
| 能力組 | Web 基線 |
| --- | --- |
| 快速判斷 | 支持 quick experience、匿名 session、session-bound case、claim / refresh 等主鏈路；結果先完整交付，註冊與補證據為後續可選入口 |
| 正式處理 | 支持登入、配對、正式案件、梳理結果、修復旅程與 execution；remote draft 回應方提交前使用 backend blind projection |
| 先聊再判 | 支持 chat room、message、stream orchestration 與轉入梳理材料；安全狀態優先於一般 handoff |
| 讓系統更懂你 | 支持 interview、psych profile、profile 接口與頁面流；使用者面以可查看、更正、刪除的系統觀察呈現，不展示完成度或信心分數 |
| 通知與內容 | 支持 notification、content、media provider 相關接口；通知頁定位為按下一個動作排序的 action inbox |
...
2. Browser local/session storage adapter；current Web quick session key 為 `emorapy_session_id`，主站 locale key 為 `emorapy_locale`，Admin Web locale key 為 `emorapy_admin_locale`。`cj_session_id` / `cj_locale` 與更早 `mbc_session_id` / `mbc_locale` 只作 lazy migration source，讀到舊值時寫入新 key 並清理舊 key，不得再作 current storage contract。
...
7. Web 視覺與互動採 Guided Reflection：暖礦物 canvas、深松綠文字、克制陶土色 primary action、閱讀型 serif heading、細分隔線與零裝飾陰影。不得用 gradient、glass、emoji persona、AI metadata、虛假進度或「每段一張卡」替代資訊層級。
8. 每個主要頁面只突出一個當下最合理的 primary action；安全／危機 route 的安全下一步可覆蓋一般 accept、invite、共同修復或轉化 CTA。
9. responsibility ratio、reconciliation effective intent、partner invite、co-repair 與 force-solo 必須消費 backend policy；前端不得從 URL、case mode 或本地預設自行推斷放寬。
10. Admin Web 保持獨立 calm operations console；route、navigation 與 RBAC 可見性共用單一 registry，401 清 session、403 保留權限不足語義。
...
4. 主站 Web 已按 adapter / strangler pattern 消費 `@emorapy/api-client` 的 M1-M5 domain client；Web wrapper 保留 browser storage、router、toast、FormData、SSE / fetch 等平台 side effect。
...
7. Guided Reflection 只代表現行 Web 視覺與內容層基線，不改變四條產品線、route identity 或 Backend API naming；使用者面術語固定為「梳理結果／發起方／回應方／調整方向」。
8. 本輪 UI 重構的 browser evidence 可驗證公開 Consumer 頁與 mock-backed Admin login/ops/health/responsive；未以真 Backend 跑過的 authenticated Consumer、Admin destructive mutation 與完整結果資料狀態仍不得宣稱 runtime 閉環。
