...
| API adapter | Axios adapter，支援 JWT、`X-Session-Id`、`X-Locale`、request id、FormData、typed error normalize | runtime adapter、header/error tests 與 M1-M5 shared domain client 接線已落地；release / production DB parity 與外部服務證據仍待補 |
...
| notifications adapter | Expo notification permission / push token payload helper | iOS / Android dynamic import helper 已落地，避免 Web static render 載入 native push side effect；backend device token registration / revoke、Expo push sender / dispatch / receipt polling job、App token sync、logout cleanup、registration-time token rotation revoke 與 notification response landing handler 已接線，真 provider delivery / 真機 cold-start landing 證據待補 |
...
| linking / lifecycle / telemetry adapters | Deep Link、AppState、safe telemetry helper | Deep Link target resolver 已保留 chat room、chat invite、profile interview/story、Quick collaborative / result 等安全子路由，Chat invite accept 未登入會保存 pending href 並走 post-login resume；notification invite path 只映射到 invite landing，不直接觸發 accept；notification response route handoff、non-notification Deep Link cold-start / foreground handoff、auth failure post-login resume、lifecycle subscription unit 與 telemetry redaction 已落地；telemetry 已 non-blocking 接 `POST /api/v1/telemetry/events` safe ingest 與 `POST /api/v1/telemetry/otlp/v1/traces` CJ OTLP JSON trace ingest，並有 backend 二次清洗、最小化 persistence、Admin 聚合報表與 30d cleanup contract gate；production backend CORS 已只對這兩個 App native telemetry endpoint 允許無 `Origin` 請求，非白名單瀏覽器 Origin 仍拒絕；App observability bootstrap 已接 session start、lifecycle transition、global JS fatal / unhandled promise、OpenTelemetry provider span export 與 native crash SDK configuration status；device evidence、native crash runtime evidence / external tracing backend 待補 |

因此，當前 App 端可以稱為「CJ App M0-M5 首輪普通用戶工程已落地、M1-M5 本機 true-service probes 已通過、iOS/Android simulator/emulator native smoke 已通過」。但在 M6 strict release sign-off 前，仍不能稱為可發布完成版、TestFlight 完成、physical-device 完整驗收、真 Push delivery 完成、native crash runtime 完成或 release / production DB parity 完成。
...
| Repair | repair journey、today step、checkin、replan、dashboard | reconciliation / execution / repair-track stream | M4 |
...
| Notifications | permission / push token helper、in-app notification screen、notification landing handler 與 web skip branch test 首輪已建立 | 封裝 permission、push token 取得、登出清理、token sync、registration-time rotation revoke 與 notification response route handoff | 不得讓 Push 狀態繞過 backend notification 狀態；registration / revoke / logout cleanup / rotation revoke / landing handler 與 backend Expo push dispatch / receipt polling 已接線，但真 provider delivery、APNs sandbox 與真機 cold-start landing 證據尚未驗收 |
...
| Telemetry | safe console telemetry、token/session key redaction、backend safe ingest、CJ OTLP JSON trace ingest、最小化 persistence、Admin 聚合報表與 30d cleanup、App observability bootstrap、OpenTelemetry provider 與 native crash SDK configuration 已建立 | app version、build number、platform、screen、request id、safe error context、JS fatal / unhandled promise 口徑、OpenTelemetry provider span export、native crash provider status | 不得上傳敏感 relationship / psych / prompt payload；`POST /api/v1/telemetry/events` + `POST /api/v1/telemetry/otlp/v1/traces` + `GET /api/v1/admin/reports/app-telemetry` + Sentry SDK configuration 只代表 safe observability first pass；production CORS 只允許這兩個 telemetry ingest endpoint 接受 App native 無 `Origin` 請求，不代表其他 API 可繞過 Origin 白名單；不代表 native crash runtime evidence、external tracing backend 或長期 crash-free sessions 已完成 |
