...
write_metadata() {
  ...
  local exit_code="${2:-}"
  {
    ...
    printf 'main_web_url=%s\n' "${MAIN_WEB_URL:-https://mother-bear-court.vercel.app}"
    printf 'admin_web_url=%s\n' "${ADMIN_WEB_URL:-https://frontend-admin-sigma-virid.vercel.app}"
    printf 'backend_base_url=%s\n' "${BACKEND_BASE_URL:-unset}"
    printf 'frontend_base_url=%s\n' "${FRONTEND_BASE_URL:-unset}"
    printf 'admin_base_url=%s\n' "${ADMIN_BASE_URL:-unset}"
    printf 'origin=%s\n' "${ORIGIN:-unset}"
