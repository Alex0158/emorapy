...
**狀態**：仍在待處理；`v1.5.0` containment baseline 已由 exact `main@a685db36` 部署至 Production。CI run `29246076104` 與 Production run `29246324704` 均成功，三端 version、migration/backfill、release gate、exact AI ledger/cost breakdown 與 product-state audit 已取證。2026-07-13 本地 release branch 已實作 Wave 0B containment：shared/formal private-derived controls hard-off、Universal Shared Renderer 不接收 hidden controls、`chat_room / chat_channel` AI stream 逐 payload durable exact-scope revalidation；但尚未合併、跑 exact-main CI 或部署，Production 不得視為已有這三項修正。per-owner strategy isolation、purpose/versioned authorization、adaptation-mode consent、durable Safety Router、cross-case memory、legacy data lifecycle、live two-party 與 App native evidence仍未閉環，故不得把本次 containment release 寫成整體私人上下文治理閉環
...
7. per-owner Strategy Compiler、`shared_mediation_adaptation` 與 formal purpose split、共同房間 adaptation-mode consent、durable Safety Router仍是 P0；`chat_room / chat_channel` AI stream active-event逐 payload durable revalidation已在本地 release branch 實作，但 exact-main / Production evidence未完成。
...
「AI 看得到、對方看不到原文」只解決直接披露，未解決 shared output 的可觀察 side channel、秘密證據、推斷性傷害或撤權競態。2026-07-13 source audit 亦確認 aggregated private strategy request、hidden-control formal renderer 與 AI stream payload gate仍存在。後續方案因此更新為：
...
### 2.5 Wave 0B 本地實作狀態（尚未部署）

1. Shared mediation resolver 目前固定回 universal baseline，不查 private message，也不呼叫 aggregated strategy provider；Private Analyst 的 owner-only support path保留。
...
4. Wave 0B focused evidence 為 backend build、11 suites / 137 tests、`--detectOpenHandles` 19 tests 與 targeted lint 通過；broader branch regression另含 backend full unit 193 suites / 2364 tests、integration 5/7 suites passed（2 skipped）、81/120 tests passed（39 skipped）、fresh PostgreSQL DB flow 1 suite / 38 tests passed，以及 production-like true-service（`AI_MOCK=true`；真 HTTP/backend/fresh DB + authenticated local TLS SMTP sink）通過。這些只證明 branch-level containment及本機真服務主鏈，不證明外部 AI / 真 SMTP provider，亦不替代 exact-main CI、真 Redis cross-instance race / sustained load、credential-backed live two-party或 Production release evidence。
5. 逐 AI delta durable DB/auth check會增加讀取負載；在 per-owner compiler、雙層 consent及 deterministic merge完成前，shared dynamic controls必須維持 hard-off。
...
- 在 per-owner compiler、雙層 consent 與 deterministic merge 未完成前，Production 的 `shared_mediation_adaptation` 動態 controls預設關閉；Private Analyst 仍可完整讀取本人私密脈絡並提供 owner-only support / capsule draft，不影響私聊價值。
...
### Wave 2 — Private Analyst 與 mediation controls（Private Analyst baseline deployed；shared controls P0 未完成）

- Private Analyst / private reply 使用 owner-scoped stream。
- 現行 aggregated Strategy extraction不得當作目標架構；每位 owner 必須各自使用隔離 Strategy Compiler，單一 model request只見一位 owner 的 private bundle。之後由 backend deterministic minimum-disclosure merge，並要求 purpose grant + adaptation-mode雙層 consent；完成前 shared dynamic controls維持關閉。
