...
2. **建立Session**（自動）→ 生成 `guest_*` Session 供快速體驗授權
3. **選擇模式**：
...
5. **提交案件** → 直接進入案件提交流程（無需選擇案件類型，AI自動判斷）
6. **（可選）上傳證據** → 判決生成前可補傳；判決完成後關閉上傳
7. **獲得判決** → 立即顯示判決結果（含責任分比例）
...
- 不支持和好方案生成（可選，需要註冊）
- 不支持執行追蹤（可選，需要註冊）
 - Session 初始有效期 24 小時，判決成功後延長至 7 天
...
- **介面設計**：
  ...
  - **輸入框設計**：
    ...
    - 字數要求：角色A最少30字，角色B最少10字（可留空），最多2000字
    - 實時字數統計
    - 支持Markdown格式（可選）
  - **提交按鈕**：
    - 角色A填寫完成後，顯示「提交案件」按鈕
...
**MVP 階段說明（等待優化內容）**：  
等待優化內容（推薦案例、關係技巧文章、簡單測試題）在設計與後端已預留：Content API 與 `ContentItem`/`CaseContentLink` 表存在，但 MVP 階段無種子數據，且快速體驗/審理中頁面未接該 API。此功能列為**後續迭代**，待有內容與運營需求時再補齊種子數據並在等待頁展示。
