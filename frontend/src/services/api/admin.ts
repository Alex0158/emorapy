...
export function getAdminToken(): string {
  ...
  try {
    // 優先 sessionStorage，降低長期暴露風險；兼容舊版 localStorage。
...
export function subscribeAdminTokenChanges(onStoreChange: () => void): () => void {
...
function normalizeStatsRow(value: Partial<AdminJobStatsRow> | undefined): AdminJobStatsRow {
  return {
...
function normalizeStatsMeta(value: Partial<AdminJobStatsMeta> | undefined): AdminJobStatsMeta {
  return {
...
/**
 * 舊版後端可能缺少 rateBase / statsMeta，先做容錯標準化再進入圖表層。
 */
export function normalizeAdminJobStatsData(
  ...
  return {
    ...
    totals: {
      ...totals,
      avgDurationMs:
        Number.isFinite(avgDurationMsRaw) && avgDurationMsRaw >= 0 ? avgDurationMsRaw : 0,
    },
...
export const adminApi = {
  async login(payload: { email: string; password: string }): Promise<AdminLoginData> {
    const response = await request.post<ApiResponse<AdminLoginData>>('/admin/login', payload);
  ...
  async getMe(): Promise<AdminMeData> {
    ...
    return {
      admin: {
        ...
        permissions: Array.isArray(payload.admin.permissions) ? payload.admin.permissions : [],
      },
    };
  },

  async getJobStats(query: AdminJobStatsQuery = {}): Promise<AdminJobStatsData> {
    const response = await request.get<ApiResponse<AdminJobStatsData>>('/admin/jobs/stats', {
      params: query,
      headers: getAdminAuthHeaders(),
    });
  ...
  async getHealthDetailed(): Promise<AdminHealthDetailedData> {
    const response = await request.get<ApiResponse<AdminHealthDetailedData>>('/admin/health/detailed', {
      headers: getAdminAuthHeaders(),
    });
  ...
  async listJobs(): Promise<{ jobs: AdminJobListItem[] }> {
    const response = await request.get<ApiResponse<{ jobs: AdminJobListItem[] }>>('/admin/jobs', {
      headers: getAdminAuthHeaders(),
    });
    return (response.data as ApiResponse<{ jobs: AdminJobListItem[] }>).data;
  },

  async triggerJob(jobKey: string): Promise<{ jobKey: string; triggeredAt: string; status: string; note: string }> {
    const response = await request.post<ApiResponse<{ jobKey: string; triggeredAt: string; status: string; note: string }>>(
      `/admin/jobs/${jobKey}/trigger`,
      {},
      { headers: getAdminAuthHeaders() }
    );
    return (response.data as ApiResponse<{ jobKey: string; triggeredAt: string; status: string; note: string }>).data;
  },

  async listConfigs(params?: { limit?: number; offset?: number }): Promise<AdminListResponse<AdminConfigItem>> {
    const response = await request.get<ApiResponse<AdminListResponse<AdminConfigItem>>>('/admin/configs', {
      params,
      headers: getAdminAuthHeaders(),
    });
  ...
  async upsertConfig(payload: {
    ...
    const response = await request.put<ApiResponse<{ item: AdminConfigItem; runtime?: { jobsEnabled: boolean } }>>(
      '/admin/configs',
      payload,
      { headers: getAdminAuthHeaders() }
    );
    return (response.data as ApiResponse<{ item: AdminConfigItem; runtime?: { jobsEnabled: boolean } }>).data;
  },

  async listUsers(params?: { q?: string; limit?: number; offset?: number }): Promise<AdminListResponse<AdminAppUserItem>> {
    const response = await request.get<ApiResponse<AdminListResponse<AdminAppUserItem>>>('/admin/users', {
      params,
      headers: getAdminAuthHeaders(),
    });
  ...
  async getUserDetail(userId: string): Promise<{ user: unknown }> {
    const response = await request.get<ApiResponse<{ user: unknown }>>(`/admin/users/${userId}`, {
      headers: getAdminAuthHeaders(),
    });
  ...
  async updateUserStatus(
    userId: string,
    payload: { action: 'lock' | 'unlock' | 'deactivate' | 'activate'; lockMinutes?: number }
  ...
  async listAuditLogs(params?: {
    ...
    const response = await request.get<ApiResponse<AdminListResponse<AdminAuditLogItem>>>('/admin/audit-logs', {
      params,
      headers: getAdminAuthHeaders(),
    });
  ...
  async downloadAuditLogsCsv(params?: Record<string, string | number | undefined>): Promise<Blob> {
  ...
  async getReportOverview(): Promise<AdminReportOverviewData> {
    const response = await request.get<ApiResponse<AdminReportOverviewData>>('/admin/reports/overview', {
      headers: getAdminAuthHeaders(),
    });
  ...
  async getReportFunnel(): Promise<AdminReportFunnelData> {
    const response = await request.get<ApiResponse<AdminReportFunnelData>>('/admin/reports/funnel', {
      headers: getAdminAuthHeaders(),
    });
  ...
  async getCustomReport(metrics: string[]): Promise<{ metrics: Record<string, number> }> {
    const response = await request.post<ApiResponse<{ metrics: Record<string, number> }>>(
  ...
  async getInterviewRuntimeConfig(): Promise<AdminInterviewRuntimeConfigData> {
    const response = await request.get<ApiResponse<AdminInterviewRuntimeConfigData>>('/admin/runtime/interview', {
      headers: getAdminAuthHeaders(),
    });
  ...
  async setFeatureFlags(flags: Record<string, unknown>): Promise<{ item: AdminConfigItem }> {
    const response = await request.put<ApiResponse<{ item: AdminConfigItem }>>(
      '/admin/feature-flags',
      { flags },
      { headers: getAdminAuthHeaders() }
  ...
  async listAdminUsers(params?: { q?: string; limit?: number; offset?: number }): Promise<AdminListResponse<AdminAdminUserItem>> {
    const response = await request.get<ApiResponse<AdminListResponse<AdminAdminUserItem>>>('/admin/admin-users', {
      params,
      headers: getAdminAuthHeaders(),
    });
  ...
  async createAdminUser(payload: {
    ...
    const response = await request.post<ApiResponse<{ item: AdminAdminUserItem }>>('/admin/admin-users', payload, {
      headers: getAdminAuthHeaders(),
    });
