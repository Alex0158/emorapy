...
**狀態**：仍在待處理；`v1.5.0` 產品、backfill 與 release recovery baseline 已進入 `main@8bb9f04`。Production run `29240717786` 的 exact deploy、Main/Admin staged verify/promote、三端 version gate、migration/backfill idempotency、health及兩段 mutating smoke 均成功，但 final product-state audit 的既有 8-query fan-out 撞到 session pool 上限；修正後自動 rollback 完整恢復 Backend `c031057`、Main/Admin `95fa8a9`，DB 沒有 down migration。`dea5520` 已把 audit 收斂為 sequential reads並以同一 Production env read-only 驗證。該 run logs 另揭露 AI request ledger 因 Prisma delegate casing 錯誤沒有真正落庫；`24c0eb7` 已修正 generated-client shape，`b3f3716` 已把 exact synthetic case ledger 與 Admin breakdown 加入 release smoke。全部仍須通過新 exact-main workflow。第二輪產品／安全審核新增 process-control side channel、purpose/versioned authorization、private safety pause、AI stream active-event boundary、cross-case memory 與 legacy data lifecycle 缺口，故不得把本次 containment release 寫成整體私人上下文治理閉環
...
`main@c031057` 已包含以下能力；release recovery hardening 不改變下列產品／隱私契約，Production 是否完整承接仍須以成功的 exact-main workflow 與 live version 取證：
...
1. 產品／backfill baseline 已於 `main@c031057` 合併，exact-main `CI` push run `29238079115` 全綠；此項只解除 source/CI blocker，不單獨代表三端 Production 已發布。
2. `20260712210000_add_chat_context_domain_foundation` 與 `20260713090000_add_context_authorization_active_unique` 已落到 Production DB；run `29238278283` 的 Railway pre-deploy 重新驗證 migration parity / readiness 並保存 artifact，不做 down migration。
3. 先前 partial state 的 11 個 shared、18 個 private channel 與 50 個未分類 message，已由 run `29238278283` 的 bounded、parameterized bulk write 完成 assignment；apply 後 remaining、resolution failure、quarantine、orphan 與 invariant drift 均為 0。後續正式 workflow 必須以 idempotent 0-update／0-remaining 重驗，任何 blocker 都要停止 Web promote。
...
5. 全量單元／整合／client tests 及本地 fresh PostgreSQL / Redis runtime 已通過；正式完成判定仍要保存 release-gate evidence，並執行可用憑證容許的 two-party canary。App native lifecycle 證據維持獨立 release scope，不能由 Web Production 發布代替。
6. cross-case personal/joint memory、共同記憶雙方版本批准及完整資料管理仍未實作；本次安全邊界發布不得把此 P1 產品能力寫成已完成。
7. Production workflow 固定為 `expand migration -> Railway backend -> chat-context dry-run -> apply -> read-only privacy audit -> staged Web/Admin -> exact-SHA verify -> promote -> release gate`。run `29238278283` 證明 backend/backfill 可成功，但 Web/Admin 未閉環；migration log、backfill counts、legacy privacy audit、三端 version 與 release gate 缺一不可，不得因部分 job 成功就宣稱已發布。
...
13. `24c0eb7` 已以 `Pick<PrismaClient, 'aIRequestLedger'>`、typed database module 與 runtime-shaped mocks 修復；`b3f3716` 令 release smoke 只以本次 synthetic case 的 exact `case_judgment / quick_single / judgment_draft` scope 驗 `succeeded/completed/tokens/cost`，再查 Admin costs ledger。verifier 只輸出低敏狀態與布林值，仍須以新 Production run 及新 deployment warning count=0 取證。
...
2. Shared Mediator 採 universal trauma-informed baseline，所有房間都具分段、先詢問及暫停選項。若要由 private context 動態改變共同節奏，必須把「另一方可能察覺流程變化」視為 disclosure，以獨立、purpose-scoped、versioned authorization 明示批准；shared mediation 與 formal delivery 不得共用一個 participant enum。
...
5. Capsule saved draft/revision/discard/re-authorize、owner-facing usage receipt、structured audit refs、retention/export/delete、per-owner retrieval、solo/joint Repair resolver，以及 PersonalMemory / 雙方 exact-approved JointMemory 均保持後續 scope。raw transcript、責任比例、單方指控、safety label及人格／創傷診斷不得自動跨案件記憶。
...
`main@c031057` 已 additive 新增 channel、capsule、authorization、analysis request/approval endpoints，並擴充 shared contracts、`@emorapy/api-client`、Web 與 App 消費面；legacy room message / `included_message_ids` 路徑仍保留相容窗口。本地與 Production DB backfill 均已通過，完整三端 release artifact、舊 client 相容觀察與 deprecation 由本文件的最新 release evidence 追蹤。
...
### Wave 5 — Release migration（Production data/backend passed；三端 closure 由正式 workflow 裁決）
...
- Production workflow 順序固定為 expand migration → Railway backend → chat-context dry-run/apply/audit → staged Web/Admin → exact-SHA verify → promote → release gate；歷史 partial run 不能代替後續成功的同一 exact-main SHA 完整閉環。
