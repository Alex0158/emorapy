...
| `POST /api/v1/auth/send-verification-code` | `email`、`type(register/verify_email)` | `data.expires_in`、`data.resend_after`（只在 email provider 接受後返回；無需寄送的 verify_email no-op 亦回相同 neutral envelope） | `RATE_LIMIT_EXCEEDED`、`INVALID_EMAIL`、`EMAIL_EXISTS`、`EMAIL_DELIVERY_UNAVAILABLE` | 寫入 OTP HMAC challenge；provider 失敗即作廢 | `/auth/register`、既有帳戶驗證 |
