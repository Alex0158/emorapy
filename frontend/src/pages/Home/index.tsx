 ...
 * 保留: AdaptiveDashboard 子組件
 * 保留: Home.css（子組件和 section 佈局）
 */

import { lazy, Suspense, useMemo } from 'react';
import { useNavigate } from 'react-router-dom';
import { Rocket, Heart, Lightbulb, CheckCircle, FileText, Ear, ShieldCheck } from 'lucide-react';
import { Button } from '@/components/ui/button';
import SEO from '@/components/common/SEO';
import AnimatedWrapper from '@/components/common/AnimatedWrapper';
const ListeningDemo = lazy(() => import('./components/ListeningDemo'));
      ...
      <div className="home-page">
          ...
          <section id="main-content" className="hero-section relative overflow-hidden" aria-label={t('home.hero.ariaLabel')}>
            <div className="hero-content">
              <AnimatedWrapper animation="slide" direction="right" delay={0} duration={300} className="hero-text">
                ...
                <div className="hero-boundaries" aria-label={t('home.hero.boundariesAria')}>
                  <span><ShieldCheck className="size-4" aria-hidden="true" />{t('home.hero.boundaryPrivate')}</span>
              ...
              <AnimatedWrapper animation="fade" delay={100} duration={300} className="hero-image">
                <div className="hero-listening-card" aria-label={t('home.hero.cardAria')}>
                  <div className="hero-card-seal" aria-hidden="true">e</div>
                  <div className="hero-card-label">
                    <Ear className="size-4" aria-hidden="true" />
                    {t('home.hero.cardLabel')}
                  </div>
                  <p className="hero-card-quote">{t('home.hero.cardQuote')}</p>
                  <div className="hero-card-answer">
                    <span>{t('home.hero.cardAnswerLabel')}</span>
                    <strong>{t('home.hero.cardAnswer')}</strong>
                  </div>
                </div>
              </AnimatedWrapper>
            </div>
          ...
          <div className="container">
            ...
            <div className="features-grid">
              {features.map((feature) => (
                <div key={feature.titleKey} className="home-feature-card" aria-label={t(feature.ariaKey)} tabIndex={0} role="article">
        ...
        <section className="manifesto-section" aria-labelledby="manifesto-title">
          <div className="manifesto-inner">
            <p className="manifesto-eyebrow">{t('home.manifesto.eyebrow')}</p>
            <h2 id="manifesto-title" className="manifesto-title">{t('home.manifesto.title')}</h2>
            <p className="manifesto-hand">{t('home.manifesto.hand')}</p>
          </div>
        </section>
