...
describe('request', () => {
  ...
  describe('interceptors', () => {
    ...
    it('response success=false 且缺少錯誤欄位時應使用 fallback', async () => {
      ...
      ).rejects.toMatchObject({ code: 'API_ERROR', message: 'common.requestFail' });
    });
    ...
    it('400 + INVALID_SESSION_ID 應 clear + refresh 並 warning', async () => {
      await expect(
        onError({
          response: {
            status: 400,
            data: { error: { code: 'INVALID_SESSION_ID', message: 'session-bad' } },
    ...
    it('400 + SESSION_ID_REQUIRED 刷新成功且無 message 時應使用 fallback warning', async () => {
      ...
      expect(mockMessageWarning).toHaveBeenCalledWith('common.sessionExpiredRefreshed');
    });
    ...
    it('400 + INVALID_SESSION_ID 當 refresh 拋錯時應走 catch 顯示 error', async () => {
      ...
      expect(mockMessageError).toHaveBeenCalledWith('error.session.expiredHint');
    });
      ...
      await expect(
        onError({
          response: {
            ...
            config: { url: '/cases/case-1', headers: { 'X-Session-Id': 'sid-1' }, params: {} },
          },
          config: {},
          message: 'not found',
        })
