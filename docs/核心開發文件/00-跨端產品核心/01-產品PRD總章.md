...
2. 私人內容只有在本人明示同意後，才可轉成無原因、不可反推秘密的程序控制，用於調整共同對話的節奏、問題數量、詢問方式與暫停選項。
3. Shared Mediator 不得讀私人原文或原因；需要共同討論的內容必須先形成由本人預覽、編輯及批准的 versioned Context Capsule。
4. 正式梳理的事實、可信度、責任、比例與結論只能由共同可見材料或批准用於該次梳理的 capsule 支持；私人背景只可在獨立 delivery 階段調整表達。
...
正式架構決策見 `EMO-ADR-010`；`codex/private-context-consent-boundary` release candidate（基於 `origin/main@95fa8a9`）已由 Backend、Web 與 App 承接 channel、capsule、exact approval、evidence-only Judgment / Repair 邊界及相關 release hardening。本地全量測試、乾淨文件 gate、exact-image build、空白 PostgreSQL / Redis migration、backfill dry-run / apply 與 legacy privacy audit 已通過；但在分支合併、exact main SHA CI 及 `Production Deploy and Verify` 成功前，仍只屬候選版本，不得視為 Production completion。剩餘 Production 證據與 cross-case memory / legacy data lifecycle 由 [Chat 私密上下文待辦](../07-待處理問題與治理/待處理/Chat私密上下文與共同調解隔離重構待辦-2026-07-12.md) 追蹤。
