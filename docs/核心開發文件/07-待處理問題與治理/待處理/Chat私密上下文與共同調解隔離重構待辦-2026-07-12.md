...
**狀態**：仍在待處理；`v1.5.0` 產品與 backfill baseline 已進入 `main@c031057`。Production run `29238278283` 已完成 additive migrations、50 筆 message assignment、privacy audit 及 backend `c031057` 切流，但主站 staged verify 因 Vercel CLI 參數漂移停止，Admin 未部署，Railway rollback 又因 GraphQL 回傳型別漂移未執行；因此該 run 只屬部分發布事故證據，不是三端閉環。發布完成與否只由後續成功的 exact-main workflow artifact、三端 live version 與 release gate 裁決；cross-case memory 與 legacy data lifecycle 持續保持活躍
