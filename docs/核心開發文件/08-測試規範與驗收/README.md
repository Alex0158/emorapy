...
6. `scripts/`
7. `mobile/package.json`、`mobile/app`、`mobile/src/platform`、`mobile/scripts`（App route / feature / true-service / platform contract、native readiness、release evidence 與 completion audit gate）

上述入口目前覆蓋 Backend、Web、Admin Web、跨站真服務、發布/手動回歸腳本與 App M0-M6 測試/證據 gate。`mobile/` 已有 route / feature / true-service / platform / Maestro static / native readiness / Android readiness / release evidence / completion audit 入口；這些入口可證明 App 工程基線、contract wiring 與 release blocker 狀態，但不等於 TestFlight、真機、push provider delivery 或 production native crash runtime 已完成。不得把 Web/Admin 測試通過推斷為 App 已驗收，也不得把 App dry-run、local true-service、simulator 或 emulator evidence 外推為 M6 strict release sign-off。

本目錄不單獨定義以下產品正式語義：

1. 功能是否存在
2. 頁面哪裡是正式主入口
3. API 屬於已使用、待承接、候選廢棄或已確認廢棄
4. 業務流程的正式主鏈是什麼
