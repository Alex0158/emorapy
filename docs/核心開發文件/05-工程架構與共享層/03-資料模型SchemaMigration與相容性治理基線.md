...
| Migration history | `backend/prisma/migrations/*/migration.sql` 與 `migration_lock.toml` 已存在；expand-only `20260712210000_add_chat_context_domain_foundation` 建立 Chat channel/context/analysis models，但刻意不在 migration 內 backfill legacy messages | migration source已納入 release-blocking catalog；`a685db36` Production workflow已完成 27 migrations無 pending、16/16 required parity、backfill及 artifact保存 |
...
| Migration | 分類 | 影響模型 | 相容性與 gate |
| --- | --- | --- | --- |
| `20260712210000_add_chat_context_domain_foundation` | Additive With Backfill + Contract / Privacy / Safety Sensitive | 新增 `ChatChannel`、participant private-context preference、nullable `ChatMessage.channel_id`、deny-by-default `ChatMessage.ai_context_eligible`、Context Capsule/Authorization、Analysis Request/Participant Approval、Context Use Audit 及 exact hash/policy FK/unique constraints；DB partial unique index 保證每 room 只有一個 active Analysis request；新 room history default 收窄為 `share_from_join_time` | migration 不重新詮釋 legacy rows，`channel_id` 在 expand 期保持 nullable，legacy eligibility 預設 false。`backfill-chat-context-channels.ts` 預設 dry-run；只有分類為 shared 的 legacy human `user_text` 可明確標記 reusable，legacy private / summary / AI / system 全部 display-only；legacy `summary_only` 保持 private + review-required，不自動建 capsule；`audit-private-context-legacy-data.ts` 只讀並核對 eligibility 分組。首次 apply 的 message assignment timeout以 bounded parameterized bulk write roll-forward且不 down-migrate；Production run `29246324704` 已重驗 assignment、idempotency、remaining、quarantine、orphan與 audit均無異常。這只證明 schema/backfill release，不代表 legacy data rights或整體私密治理完成 |
| `20260713090000_add_context_authorization_active_unique` | Constraint Hardening + Privacy Sensitive + Operational Risk | `ContextAuthorization` 以 capsule、subject、purpose、audience、target type、target ID 作 unrevoked exact grant identity；partial unique index 補上跨 process idempotency。既有 `roleA` / `roleB` / `aiMediator` active singleton 已由 2026-02 partial unique migrations保護，不重複建 index | migration 先 aggregate preflight；任何 duplicate exact grant 直接失敗且不更新／刪除資料。index build 設 5 秒 lock timeout、60 秒 statement timeout並有 catalog postcondition；runtime P2002 只可回讀同一 exact、未撤銷、未過期 grant。Production run `29246324704` 已重驗 migration status、legacy duplicate audit、fresh release parity與 runtime readiness；failed deployment log只屬歷史事故證據 |
| `20260713201500_add_chat_safety_router_state` | Additive Safe + Privacy / Safety Sensitive | 新增 closed enum `ChatSafetyRouterAction`及 per-room/per-owner `ChatSafetyRouterState` current state；只保存 action、policy/state version與時間，不保存 raw text、topic、diagnosis或reason | 已加入 `RELEASE_BLOCKING_MIGRATIONS`；private analyst須在 external provider前 durable activate，shared/formal/joint gates fail closed。Production需套 migration並以 `ops:db:status`、release gate及雙人 negative test取證；套用前只可標記本地已實作 |
