...
| --- | --- | --- | --- |
| Storage | SecureStore adapter、auth/session bootstrap、Emorapy current key / legacy `cj.*` lazy migration 與 unit gate 已建立 | 封裝 Expo SecureStore，支援 token / session restore / clear / device metadata；current key 使用 `emorapy.*`，legacy `cj.*` 只作讀取遷移與清理 | 不得在 screen 中直接讀寫 SecureStore，不得長期保存 case / psych 正文；不得把 legacy `cj.*` key 當 current App storage contract |
