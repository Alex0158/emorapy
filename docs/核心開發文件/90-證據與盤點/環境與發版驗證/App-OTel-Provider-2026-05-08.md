...
| `npm --prefix mobile run release:completion:audit` | 已通過 / not complete report | 已驗證 iOS simulator native Maestro evidence、Android native evidence、native ImagePicker picker-cancel evidence、本 OpenTelemetry provider evidence、release DB parity evidence 與 telemetry runtime evidence；仍列出 EAS / TestFlight / 真機 / provider delivery / native crash runtime blockers |
| `npm --prefix mobile run release:completion:audit:strict` | 預期失敗 | strict mode 已確認 OpenTelemetry provider evidence 在 verified 區塊；失敗原因是 10 個 release completion blockers 未清零，外部交接另由 status / handoff report 暴露 completion blockers 與 prerequisite-only blockers |
