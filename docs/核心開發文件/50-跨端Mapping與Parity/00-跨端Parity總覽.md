...
**文檔類型**：正式規格
**覆蓋範圍**：跨端產品能力到 Web / App / Backend / API / DB / 共享層的映射與 Parity 缺口總覽
...
| App 尚未承接 CJ 主產品路由 | 功能 Parity | 先裁決 App 首輪能力矩陣，再開發 |
| SecureStore token / session restore 未定義 | 平台差異 | 建立 App auth/session 設計任務 |
| Push token / notification sync 未定義 | 平台差異 + backend 影響 | 建立 notification / push schema/API 評估任務 |
| Deep Link 授權失敗回退未定義 | 平台差異 | 建立 route / auth failure UX 任務 |
| App 測試與 smoke gate 未定義 | 工程治理 | 建立 App 測試規範與 CI 任務 |
