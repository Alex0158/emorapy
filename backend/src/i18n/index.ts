...
const zhTWByCode: Record<string, string> = {
  ...
  NOT_FOUND: '資源不存在',
  EMAIL_EXISTS: '郵箱已存在',
  EMAIL_NOT_VERIFIED: '請先完成郵箱驗證',
  EMAIL_DELIVERY_UNAVAILABLE: '郵件服務暫時不可用，請稍後再試',
  REGISTRATION_PROOF_INVALID: '註冊驗證已失效，請重新驗證郵箱',
  REGISTRATION_PROOF_EXPIRED: '註冊驗證已過期，請重新驗證郵箱',
...
const enUSByCode: Record<string, string> = {
  ...
  REGISTRATION_PROOF_INVALID: 'Registration verification is no longer valid. Please verify your email again.',
  REGISTRATION_PROOF_EXPIRED: 'Registration verification has expired. Please verify your email again.',
...
const directEnUSMap: Record<string, string> = {
  ...
  '若該帳戶存在，我們已開始處理密碼重置請求': 'If the account exists, we have started processing the password reset request',
  ...
  '註冊驗證格式無效': 'Invalid registration verification format',
