...
export function deriveAdminTokenEditorState(
  savedToken: string,
  inputToken: string
): AdminTokenEditorState {
  ...
  const inputTokenFormatInvalid =
    normalizedInputToken.length > 0 && !isLikelyAdminJwt(normalizedInputToken);
