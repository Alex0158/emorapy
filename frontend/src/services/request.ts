...
const addCancelToken = (config: InternalAxiosRequestConfig): void => {
  ...
  // 生成請求ID（如果未提供）
  const requestId =
    extendedConfig.metadata?.requestId || `${config.method}-${config.url}-${Date.now()}`;
...
function getHttpErrorFallbackMessage(
  ...
  switch (status) {
    ...
    case 429:
      return isUploadRequest(config) ? t('common.fileRateLimit') : t('common.rateLimit');
    case 503:
...
request.interceptors.response.use(
  ...
  async (error: AxiosError<ApiError>) => {
    ...
    if (response) {
      ...
      const fallbackMessage = getHttpErrorFallbackMessage(status, errorData?.code, response.config);
        ...
        case 400: {
          ...
          if (code === 'SESSION_ID_REQUIRED' || code === 'INVALID_SESSION_ID') {
            try {
              ...
              if (refreshed) {
                toast.warning(t('common.sessionExpiredRefreshed'));
              } else {
        ...
        case 401: {
          ...
          if (isRecoverableSessionErrorCode(code)) {
            const suppressGlobalSessionToast = Boolean(
              (response.config as ExtendedAxiosRequestConfig | undefined)?.metadata
                ?.suppressGlobalSessionToast
            );
            try {
              const { useSessionStore } = await import('@/store/sessionStore');
