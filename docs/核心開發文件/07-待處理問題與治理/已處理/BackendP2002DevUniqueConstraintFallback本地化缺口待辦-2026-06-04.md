...
1. `backend/src/i18n/index.ts` 的 `唯一約束違規:` dynamic translator 已改為拆分 prefix 與 detail。
2. detail 為 `未知字段` 時，en-US 輸出 `unknown field`；detail 為 `email` 等具體欄位名時保持原樣，不做不必要本地化。
3. `backend/tests/unit/utils/backend-i18n.test.ts` 已新增 `唯一約束違規: 未知字段` 與 `唯一約束違規: email` regression。
4. `backend/tests/unit/middleware/errorHandler.test.ts` 已新增 `P2002` 無 `meta.target` 且 `req.locale='en-US'` 的 response-level regression。

已驗證：

1. `npm --prefix backend test -- tests/unit/middleware/errorHandler.test.ts tests/unit/utils/backend-i18n.test.ts --runInBand`：passed，2 suites / 24 tests。
2. `npm --prefix backend run build -- --pretty false`：passed。
3. 靜態檢查 `translateBackendMessage('en-US', '唯一約束違規: 未知字段')` 不含 CJK，輸出 `Unique constraint violation: unknown field`。

剩餘邊界：

1. production P2002 行為不變，仍回 generic conflict / email duplicate message。
2. 具體 DB 欄位名不本地化，因為它們是 diagnostics / schema identifiers，不是產品文案。
