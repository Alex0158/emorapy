...
等效手動啟動時，需先確保 `redis://127.0.0.1:6379` 可用；`./scripts/start-dev.sh` 預設使用 `/tmp/emorapy-redis-dev` 作 Redis 資料目錄，可用 `EMORAPY_DEV_REDIS_DIR` 覆蓋，`CJ_DEV_REDIS_DIR` 只作 legacy fallback。啟動 backend 時用 `EMORAPY_COMMIT_SHA=$(git rev-parse HEAD)`；`CJ_COMMIT_SHA` 保留為 legacy fallback，避免本機 `/version.commitSha` 回 `unknown`。固定端口為 backend `3001`、主站 `5173`、Admin `5175`、Redis `6379`。
...
```bash
EMORAPY_MAIN_WEB_URL=<main-web-url> EMORAPY_ADMIN_WEB_URL=<admin-web-url> npm run ops:release:status
```
...
5. `ops:release:gate` 會設定 `EMORAPY_RELEASE_GATE=1` 與 legacy `CJ_RELEASE_GATE=1`，此時 `backend/scripts/check-ai-pricing-catalog.ts` 不載入本機 `backend/.env`；發布 pricing 必須來自顯式 env / `ENV_FILE` / 平台 release env，避免本機開發版 pricing 讓發布 gate 假通過。
