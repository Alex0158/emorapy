...
- `GET /api/v1/admin/reports/app-telemetry` 只讀 `app_telemetry_events` 的最小化摘要，用於 App release 排障、crash-free sessions 初始聚合輸入與 release evidence audit；CJ OTLP JSON trace ingest 也只會轉成 `app_otel_span` 摘要，不得把它當成完整產品 analytics、native crash runtime evidence、vendor trace backend 或 Web Admin UI 完成證據。
